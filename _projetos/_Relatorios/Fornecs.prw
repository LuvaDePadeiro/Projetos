...
Static Function ReportPrint(oReport,tSA2,tSC7,aOrdem)
	...
	If mv_par06 == 1
		cCondPC := " AND C7_QUJE = 0 And C7_QTDACLA = 0 And C7_RESIDUO = '' AND C7_CONTRA = '' AND C7_CONAPRO<>'B' "
	elseif mv_par06 == 2
		cCondPC := " AND C7_QUJE<> 0 And C7_QUJE<C7_QUANT AND C7_RESIDUO = '' AND C7_CONTRA = '' AND C7_CONAPRO = 'B' "
	elseif mv_par06 == 3
		cCondPC := " AND C7_QUJE>=C7_QUANT AND C7_RESIDUO = '' AND C7_CONTRA = '' AND C7_CONAPRO<>'B' "
	elseif mv_par06 == 4
		cCondPC := " AND C7_ACCPROC<>'1' And  C7_CONAPRO ='B' And C7_QUJE < C7_QUANT AND C7_RESIDUO ='' "
	ENDIF
	...
	cQuery+= " C7_LOCAL BETWEEN '"+ mv_par04 + "' AND '" + mv_par05 + "' " + ENTER
	cQuery+= cCondPC
	cQuery+= cOrdem
